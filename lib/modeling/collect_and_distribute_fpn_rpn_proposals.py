...
class CollectAndDistributeFpnRpnProposalsOp(nn.Module):
    ...
    def forward(self, inputs, roidb, im_info):
        ...
        """
        rois, scores, anchors_idx = collect(inputs, self.training)
        if self.training:
            ...
            # not matter).
            # add rois to the ground truth boxes
            json_dataset.add_proposals(roidb, rois, im_scales, crowd_thresh=0, scores =scores, anchors_idx=anchors_idx)
...
def collect(inputs, is_training):
    ...
    score_inputs = inputs[num_lvls:2*num_lvls]
    anchors_idx = inputs[2*num_lvls: ]

    # rois are in [[batch_idx, x0, y0, x1, y2], ...] format
    # Combine predictions across all levels and retain the top scoring
    rois = np.concatenate(roi_inputs)
    scores = np.concatenate(score_inputs).squeeze()
    anchors_idx = np.concatenate(anchors_idx).squeeze()
    inds = np.argsort(-scores)[:post_nms_topN]
    rois = rois[inds, :]
    return rois, scores[inds], anchors_idx[inds]
